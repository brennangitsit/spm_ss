...
%% Create inter-subject overlap map
% For each subject, average localizer masks across cross-validation folds,
% binarize at 0.5, then compute the proportion of subjects showing activation
% at each voxel. All volumes are mapped to the first subject's space.
ssPM1=['Overlap',ext1];
p=0;
for n=1:ss.n,
    idx=find(ss.PV(n,:));
    ...
    for k=1:numel(idx),
        a1=ss.VN(idx(k));
        if n==1, b1=spm_read_vols(a1);  % first subject defines the reference space
        else b1=reshape(spm_get_data(a1,pinv(a1.mat)*tXYZ),e0(1).dim);
        end
        p1=p1+(b1>0)/numel(idx);  % average binary activation across folds
    end
    p=p+(p1>.5);  % count subjects with >50% of folds active
    if n==1,
        % initialise 4D localizer volume (one frame per subject)
        ...
        % build world-coordinate grid for resampling across subjects
        [XYZ{1},XYZ{2},XYZ{3}]=ndgrid(1:a1.dim(1),1:a1.dim(2),1:a1.dim(3));
        tXYZ=reshape(cat(4,XYZ{:}),[],3)';tXYZ=a1.mat*cat(1,tXYZ,ones(1,size(tXYZ,2)));
    end
    spm_write_vol(e0(n),p1);
    if 1 % debug: write per-subject localizer voxel list
        fname='Localizer.csv';
...
% save the overlap map as a proportion (0 to 1)
...
%% Homologous parcellation: make the overlap map bilaterally symmetric
% When ss.homologous is set, flip the overlap map across the sagittal plane
% (dimension 1 = left-right) and average with the original. This produces
% a symmetric map so that the subsequent watershed yields homologous parcels
% in both hemispheres.
if ss.typen==2 && isfield(ss,'homologous') && ss.homologous,
    disp('Creating homologous (bilaterally symmetric) overlap map...');
    overlap_data = spm_read_vols(e1);
    flipped_data = flip(overlap_data, 1);  % flip along x-axis (sagittal)
    homologous_data = (overlap_data + flipped_data) / 2;
    spm_write_vol(e1, homologous_data);
    % reload volume header to ensure clean state for spm_smooth
    e1 = spm_vol(ssPM1);
end

%% Smooth the overlap map (GcSS only)
% Smoothing controls the spatial scale of the subsequent watershed
% parcellation. The kernel is set by ss.smooth (mm FWHM).
if ss.typen==2,
    ssPM2=['sOverlap',ext1];
    spm_smooth(ssPM1,ssPM2,ss.smooth*[1,1,1]);  % use filename string, not struct
    ss.PM2=ssPM2;
end
fprintf(1,'\n');
ss.PM1=ssPM1;

%% Define fROIs via watershed parcellation (GcSS) or load manual ROIs (mROI)
if ss.typen==2,
    ss.PM=['fROIs',ext1];
    if isfield(ss,'homologous') && ss.homologous
        disp('GcSS defining homologous ROIs. Please wait...');
    else
    ...
    if ~isempty(XM),
        ...
        tXYZ=reshape(cat(4,XYZ{:}),[],3)';tXYZ=cat(1,tXYZ,ones(1,size(tXYZ,2)));
        tXM=reshape(double(spm_get_data(XM,pinv(XM.mat)*ss.refspace.masks{1}.mat*tXYZ)>0),ss.refspace.masks{1}.dim(1:3));
    else tXM=1;
    end
    % watershed on the negated smoothed overlap map: minima become basins
    % only voxels above the overlap threshold (and within explicit mask) are parcellated
...
for n=1:ss.n
    ...
    % QA image: parcel labels in subject space
...
for nroi=1:nrois,
    ...
    for n=1:ss.n
        % find voxels belonging to this ROI in subject n's space
        ...
        % read localizer weights and effect data at ROI voxels
        idxk=ss.PV(n,:)>0;
        idxk1=find(idxk,1);
        tN=spm_get_data(ss.VN(idxk),xyz);  % localizer mask values
        tvy=reshape(ss.VY(idxk,:),[],1);
        % read effect data, handling different spatial alignments
        ...
        % weighted average across voxels within ROI (weighted by localizer mask)
    ...
    sN=mean(N>1e-4,1);  % proportion of subjects with data in this ROI
    if sN>0,
